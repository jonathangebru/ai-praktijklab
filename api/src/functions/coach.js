 ...
 * POST /api/coach
 *
 * Single-shot gestructureerde feedback voor drie modes:
 *   · reflection — docent's reflectie-antwoord
 *   · rubric     — docent's lesopzet + zelfscore
 *   · prompt     — docent's prompt vs modelprompt
 *
 * Response: { ok: true, feedback: string, suggestions: string[] }
 * Caps: 1000 input tokens (~4000 chars), 600 output tokens.
...
app.http("coach", {
  ...
  handler: async (req, context) => {
    ...
    return {
      status: 200,
      headers: CORS,
      jsonBody: { ok: true, feedback, suggestions, mode },
    };
  },
});
